...
class DPSGD_GNN:
    ...
    def stack_subgraphs(self, batch, max_nodes: int, max_edges: int):
        ...
        Creates fixed-size stacked tensors from NeighborLoader batch.
        Handles batch tensor creation, seed node identification, and edge remapping.
        ...
        Returns:
            ...
            num_nodes_per_graph: [batch_size] - actual num nodes per subgraph
            targets: [batch_size] - labels for seed nodes
        """
        ...
        if hasattr(batch, 'batch') and batch.batch is not None:
            ...
            # For the remaining nodes (neighbors), figure out which seed they belong to
            # Use edge connectivity: a neighbor belongs to the seed it connects to
    ...
    def compute_loss_single_subgraph(
        ...
        For node-level prediction, we only compute loss on the seed node (first node).
        This function will be vectorized by vmap.

        Args:
            params: Model parameters
            buffers: Model buffers
            x_single: [max_nodes, features] - node features (padded with zeros)
            edge_index_single: [2, max_edges] - edges (padded)
            num_nodes: scalar tensor - actual number of nodes in this subgraph
            target: scalar - label for seed node
        ...
        # Create mask for valid edges (both endpoints < num_nodes_actual)
        edge_index = edge_index_single
        valid_edge_mask = (edge_index[0] < num_nodes_actual) & (edge_index[1] < num_nodes_actual)

        # For vmap compatibility, use torch.where instead of boolean indexing
        # Replace invalid edges with (0, 0) which won't affect computation much
        ...
        # Create batch tensor with max_nodes size (all zeros for single graph)
        ...
        # Compute loss only on seed node (index 0)
        # Model outputs log_softmax, so use NLLLoss
    ...
    def compute_per_sample_gradients(self, batch, max_nodes: int = 100, max_edges: int = 200):
        ...
        # Vectorize over all inputs except params and buffers
        # in_dims: (params=None, buffers=None, x=0, edge_idx=0, num_nodes=0, targets=0)
        # randomness='different' allows dropout to use different masks for each sample
        ...
        per_sample_grads = vmap_grad_fn(
            ...
            x_stacked,
            edge_idx_stacked,
            num_nodes,  # REAL num_nodes for each subgraph
            targets
        )
    ...
    def step(self, batch, optimizer) -> float:
        ...
        # Compute per-sample gradients with vmap
        ...
        with torch.no_grad():
            ...
            # Loss on seed nodes (first batch_size nodes)
