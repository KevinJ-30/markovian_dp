...
def _validated_ogb_node_split_indices(name, split_idx, num_nodes):
    ...
    for local_name, ogb_name in (("train", "train"), ("val", "valid"), ("test", "test")):
        ...
        if not isinstance(index, torch.Tensor) or index.ndim != 1:
            raise ValueError(
                f"OGB dataset {name} has an invalid official split: {ogb_name} must be one-dimensional"
            )
        if index.dtype.is_floating_point or index.dtype == torch.bool:
            raise ValueError(
                f"OGB dataset {name} has an invalid official split: {ogb_name} must contain integer node IDs"
            )
        index = index.detach().cpu().to(torch.long)
        if torch.any(index < 0) or torch.any(index >= num_nodes):
            raise ValueError(
                f"OGB dataset {name} has an invalid official split: {ogb_name} contains out-of-range node IDs"
            )
...
def _load_cora_ml():
    ...
    coo = adjacency.tocoo()
    data = Data(
        x=torch.from_numpy(attributes.toarray()).float(),
        y=labels,
