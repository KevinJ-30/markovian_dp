...
class NodeLevelProGAP (ProGAP):
    ...
    def calibrate(self):
        ...
        self.composed_mechanism = ComposedNoisyMechanism(
            noise_scale=1.0,
            mechanism_list=[
                self.nap.mechanism,
                self.noisy_sgd
        ...
        if self.delta == 'auto':
            self.effective_delta = 0.0 if np.isinf(self.epsilon) else 1. / (10 ** len(str(self.num_train_nodes)))
            console.info('delta = %.0e' % self.effective_delta)
        else:
            self.effective_delta = float(self.delta)
            if not 0 < self.effective_delta < 1:
                raise ValueError('delta must be in (0, 1)')

        with console.status('calibrating noise to privacy budget'):
            self.noise_scale = self.composed_mechanism.calibrate(
                eps=self.epsilon, delta=self.effective_delta,
            )
