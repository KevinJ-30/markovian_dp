...
def compute_out_degree_inverse(edge_index, num_nodes, cache_file_path):
    ...
    torch.save(inverse, cache_file_path)
    return inverse
...
class subgraph_sampler(Dataset):
    def __init__(self, 
        ...
        """Store neighbors under a partition-specific caller-owned cache path."""
        ...
        if os.path.exists(file_path):
            ...
            source_nodes = self.graph_edge_index[0]
            neighbor_nodes = self.graph_edge_index[1]
            mask = self.mask
            self.ids_for_legit_rest_nodes_in_graph_device = self.ids_for_legit_rest_nodes_in_graph.reshape(-1)
            for node in tqdm(self.ids_for_legit_rest_nodes_in_graph):
                ...
                self.dict_of_nodes_neighbors[node] = neighbors[
                    torch.randperm(neighbors.numel(), device=neighbors.device)
                ].cpu()
            torch.save([self.dict_of_nodes_neighbors, self.mask.cpu()], file_path)
...
def get_subgraphs_loader(train_dataset, expected_batchsize, worker_num=0, drop_last=True, dataset_mode='train'):
    if dataset_mode == 'train':
        assert expected_batchsize <= len(train_dataset), (
            f'expected_batchsize = {expected_batchsize} > {len(train_dataset)}'
        )
    ...
    return DataLoader(
        dataset=train_dataset,
        batch_size=min(expected_batchsize, len(train_dataset)),
        shuffle=True,
        num_workers=worker_num,
        pin_memory=True,
        drop_last=False,
