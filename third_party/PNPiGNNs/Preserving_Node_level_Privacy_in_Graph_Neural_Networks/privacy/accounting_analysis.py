...
def get_std_node_dp(
    ...
    high = 1.0
    achieved, _ = computer.eps_from_noise(high, delta, show_flag=False)
    for _ in range(1024):
        if achieved <= epsilon:
            break
        high *= 2.0
        ...
        raise RuntimeError('could not bracket a safe HeterPoisson noise multiplier')
    low = high / 2.0
    while True:
        midpoint = (low + high) / 2.0
        midpoint_epsilon, _ = computer.eps_from_noise(midpoint, delta, show_flag=False)
        if midpoint_epsilon <= epsilon:
            high, achieved = midpoint, midpoint_epsilon
        else:
            low = midpoint
        if high - low <= 1e-3:
            break
