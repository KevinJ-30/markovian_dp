...
flags.DEFINE_float('EncDropout', 0.5, 'Encoder dropout rate')
...
flags.DEFINE_float('dropout', 0.5, 'Dropout used for training')
...
def main(unused_argv):
    ...
    epsilon = 0
    eps_sgd = 0
    ppr_releases = min(FLAGS.ppr_num, len(train_index))
    ...
    if FLAGS.dp_ppr == True:
        ...
        delta = 2 * FLAGS.delta * ppr_releases
        composition = (lambda x: x / (2 * np.sqrt(ppr_releases * np.log(np.e + x / delta))))
        ...
        delta = 2 * ppr_releases * FLAGS.delta
        composition = (lambda x: x / (2 * np.sqrt(ppr_releases * np.log(np.e + x / delta))))
    ...
    if FLAGS.dp_sgd:
        ...
        delta_sgd = FLAGS.delta_sgd / FLAGS.privacy_amplify_sampling_rate
    ...
    start = time.time()
    topk_train = ppr.topk_ppr_matrix_ista(train_adj_matrix, FLAGS.alpha, FLAGS.eps, FLAGS.rho,
                                            train_index[:ppr_releases],
    ...
    if ppr_releases < len(train_index):
        topk_train_I = np.identity(len(train_index))
        topk_train_I = topk_train_I[ppr_releases:]
